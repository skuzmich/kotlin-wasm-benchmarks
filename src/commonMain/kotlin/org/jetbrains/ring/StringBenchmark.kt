...
// TODO: Strings are slow in Wasm. Fix it.
const val STRING_BENCHMARK_SIZE = BENCHMARK_SIZE / 10
...
open class StringBenchmark {
    ...
    init {
        val list = ArrayList<String>(STRING_BENCHMARK_SIZE)
        for (n in stringValues(STRING_BENCHMARK_SIZE))
            list.add(n)
        _data = list
        csv = ""
        for (i in 1..STRING_BENCHMARK_SIZE-1) {
